...
import { getSubtypeProfilePriceUsd } from '@/lib/subtypeProfileAccess';
import { tagMailchimpContact, subtypeMailchimpTag } from '@/lib/mailchimp';
import { useIsMobile } from '@/hooks/use-mobile';

interface ProfilePurchaseCheckoutProps {
...
const CheckoutForm: React.FC<{
  user: ProfilePurchaseCheckoutProps['user'];
  elementalType: string;
  elementalSubtype: string;
  subtypeLabel: string;
  amount: number;
  onSuccess: () => void;
  onCancel: () => void;
}> = ({ user, elementalType, elementalSubtype, subtypeLabel, amount, onSuccess, onCancel }) => {
  ...
  const paymentElementOptions = useMemo<StripePaymentElementOptions>(
    () => ({
      layout: isMobile
        ? { type: 'accordion', defaultCollapsed: false, radios: 'auto', spacedAccordionItems: true }
  ...
  const finalizePurchase = async (paymentIntentId: string) => {
    const { data, error: confirmError } = await supabase.functions.invoke('confirm-profile-purchase', {
      body: {
        paymentIntentId,
        userId: user.id,
        elementalType,
    ...
    const { error: profileError } = await supabase
      .from('user_profiles')
      .update({
        subtype_profile_unlocked: true,
        subtype_profile_purchased_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      })
      .eq('id', user.id);

    if (profileError) {
      console.error('Profile unlock update failed:', profileError);
    }

    const fullName = user.user_metadata?.full_name ?? '';

    if (user.email && data?.pdfEmailSent !== true) {
      supabase.functions
        .invoke('send-profile-pdf-email', {
          body: {
            email: user.email,
            fullName,
            elementalSubtype,
        ...
        .then(({ data: emailData, error: emailError }) => {
          if (emailError || emailData?.error || emailData?.success === false) {
            console.error(
              'Profile PDF email retry failed (non-fatal):',
              emailError?.message || emailData?.error || emailData,
            );
          }
        })
        .catch((err) => console.error('Profile PDF email retry failed (non-fatal):', err));
    }

    // Mark the buyer in Mailchimp for campaign segmentation (fire-and-forget).
    if (user.email) {
      ...
      tagMailchimpContact({
        ...
        lastName: rest.join(' '),
        elementalType,
        subtypeName: subtypeLabel,
        tags: ['profile_purchased', 'purchaser', subtypeMailchimpTag(elementalSubtype)],
        inactiveTags: ['profile_not_purchased'],
      }).catch((err) => console.error('Mailchimp purchase tag failed (non-fatal):', err));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: confirmError, paymentIntent } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/?profile-purchase=success`,
      ...
      if (paymentIntent?.status === 'succeeded') {
        ...
        toast({
          ...
          description: `Your full ${subtypeLabel} profile is now available.`,
        });
        onSuccess();
      }
    } catch (err: unknown) {
  ...
  if (succeeded) {
    return (
      <div className="text-center py-6 sm:py-8 px-1">
        ...
        <p className="text-sm sm:text-base text-gray-600">Your full Elemental Subtype Profile is unlocked.</p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
      ...
      <div className="profile-checkout-footer shrink-0 border-t border-gray-100 bg-white pt-4 mt-3 -mx-4 px-4 sm:-mx-6 sm:px-6 pb-[max(1rem,env(safe-area-inset-bottom))]">
        <div className="flex items-center justify-center gap-2 text-[11px] sm:text-xs text-gray-400 text-center mb-4">
          ...
          <span className="leading-snug">Secure one-time payment</span>
        </div>
...
const ProfilePurchaseCheckout: React.FC<ProfilePurchaseCheckoutProps> = ({
  user,
  elementalType,
  elementalSubtype,
  subtypeLabel,
  ...
  const [error, setError] = useState<string | null>(null);
  const amount = getSubtypeProfilePriceUsd();
  ...
  useEffect(() => {
    const initPayment = async () => {
      ...
      setError(null);
      try {
        const { data, error: fnError } = await supabase.functions.invoke('create-profile-payment', {
          body: {
            userId: user.id,
    ...
    initPayment();
  }, [user.id, user.email, elementalType, elementalSubtype, amount]);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-3 sm:p-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
      <div className="bg-white rounded-xl sm:rounded-2xl max-w-md w-full min-w-0 max-h-[min(90dvh,90vh)] flex flex-col overflow-hidden shadow-2xl">
        <div className="shrink-0 bg-gradient-to-r from-emerald-600 to-teal-600 p-4 sm:p-6 text-white rounded-t-xl sm:rounded-t-2xl flex justify-between items-start gap-3">
          <div className="min-w-0 pr-1">
            <div className="flex items-center gap-2 mb-1">
              <Crown className="w-5 h-5 shrink-0" />
              <h3 className="text-lg sm:text-xl font-serif leading-tight">Full Subtype Profile</h3>
            </div>
            <p className="text-white/90 text-sm truncate">{subtypeLabel}</p>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 flex-1">
              ...
              <p className="text-gray-500 text-xs mb-4">
                Ensure the <code className="bg-gray-100 px-1 rounded">create-profile-payment</code> edge function is deployed in Supabase.
              </p>
              <button
                type="button"
                onClick={onCancel}
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              >
                <CheckoutForm
                  ...
                  elementalSubtype={elementalSubtype}
...
export default ProfilePurchaseCheckout;
